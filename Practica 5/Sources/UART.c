...
static uint8_t *gbpDataToSend; /*Apuntador global al arreglo de datos que da el usuario*/

static uint16_t gwDataSize; /*Variable global para la cantidad de datos*/

static uint16_t gwIndex = 0; /*Variable global para el índice*/
...
void vfnUartInit(UartConfigInit_t * pUartUserConfig) /*Inicialización, se recibe el apuntador a la estructura*/
{
	...
	if(pUartUserConfig->pfnCallback != NULL)
	{
...
void vfnUartTransmision(uint8_t * bpDataToSend, uint16_t wDataSize) /*Función de transmisión, recibe el apuntador al arreglo de datos y el número de datos a transmitir*/
{
